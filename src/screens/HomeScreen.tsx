...
import { View, Text, ScrollView, Image, TouchableOpacity, SafeAreaView, useWindowDimensions, ActivityIndicator, Modal } from 'react-native';
...
function ProductCard({ product, onPress, onAdd, isWeb }: { product: Product; onPress: () => void; onAdd: () => void; isWeb: boolean }) {
  return (
    <TouchableOpacity
      activeOpacity={0.9}
      onPress={onPress}
    ...
    >
      <View className="relative aspect-[3/4] w-full overflow-hidden rounded-xl bg-surface-dark">
        ...
        <TouchableOpacity className="absolute top-2 right-2 flex h-8 w-8 items-center justify-center rounded-full bg-white/10">
...
export default function HomeScreen() {
  ...
  const products = supabaseProducts.length > 0 ? supabaseProducts : FALLBACK_PRODUCTS;
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <View className="mt-6 flex-col gap-4">
            <View className="flex-row items-center justify-between px-4">
              <Text className="text-xl font-bold tracking-tight text-white">New Arrivals</Text>
              <TouchableOpacity>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 24 }}>
                {products.map((product) => (
                  <ProductCard 
                    ...
                    product={product} 
                    onPress={() => navigation.navigate('ProductDetails', { product })} 
                    onAdd={() => addToCart(product, '50ml')} // Default size for home screen add
                    isWeb={false} 
                  />
                ))}
              </ScrollView>
